...
export const PaginationComponent = ({ totalElements, ...props }: Props) => {
  ...
  const paginationButtonRender = (
    ...
    if (type === "prev")
      return (
        <PaginationBtn onClick={scrollToTop} title="prev">
          Previous
        </PaginationBtn>
      );
    if (type === "next")
      return (
        <PaginationBtn onClick={scrollToTop} title="next">
          next
        </PaginationBtn>
      );
    return element;
  ...
  const renderTotal = (total: number, range: [number, number]): ReactNode => (
    <PaginationLeft>
      {range[0]} - {range[1]}{" "}
      <span style={{ marginLeft: "6px", opacity: "0.25" }}>out of {total}</span>
    </PaginationLeft>
  );
